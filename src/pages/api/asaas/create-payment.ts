...
function resolveCheckoutPhone(rawPhone: unknown): {
  normalized: string | null;
  error: string | null;
} {
  const error = validateBrazilPhoneInput(rawPhone);
  if (error) {
    return { normalized: null, error };
  }

  return {
    normalized: normalizeBrazilPhoneNational(rawPhone) || null,
    error: null,
  };
}
...
export default async function handler(
  ...
  const validVariant =
    variant === "core" || variant === "pro" ? variant : undefined;
  ...
  if (assinatura6M === true && product && plano) {
    ...
    const resolvedPhone = resolveCheckoutPhone(telefone);
    if (resolvedPhone.error || !resolvedPhone.normalized) {
      return res.status(400).json({
        status: "error",
        code: "INVALID_PHONE",
        message: resolvedPhone.error || "Digite um WhatsApp valido com DDD.",
      });
    }
    ...
    const isValidCpfCnpj =
      cpfCnpjCleaned.length === 11 || cpfCnpjCleaned.length === 14;
    const customerData = {
      name: nome,
      email,
    ...
    try {
      ...
      console.error(
        "[asaas-create-payment] Erro ao criar cliente (assinatura6m):",
        err,
      );
      return res.status(500).json({
        status: "error",
    ...
    const paymentData: AsaasPayment = {
      ...
      billingType: (paymentMethod === "CREDIT_CARD" ? "CREDIT_CARD" : "PIX") as
        | "PIX"
        | "CREDIT_CARD",
      value: valueInReais,
    ...
    try {
      ...
      return res
        .status(200)
        .json({ status: "success", paymentId: payment.id, payment });
    } catch (err) {
  ...
  if (bundleId && typeof bundleId === "string") {
    ...
    try {
      ...
      console.error(
        "[asaas-create-payment] Erro ao criar cliente (bundle):",
        err,
      );
      return res.status(500).json({
        status: "error",
    ...
    try {
      ...
      console.error(
        "[asaas-create-payment] Erro ao criar pagamento bundle:",
        err,
      );
      return res.status(500).json({
        status: "error",
  ...
  const planConfig =
    productConfig.plans[plano as "basico" | "completo" | "premium"];
  if (!planConfig) {
    return res.status(400).json({
      status: "error",
      code: "PLAN_NOT_FOUND",
  ...
  const quantityNum =
    typeof quantity === "number" ? quantity : parseFloat(String(quantity));
  ...
  try {
    ...
    console.log(
      "[asaas-create-payment] Cálculo do valor:",
      JSON.stringify({
    ...
    return res.status(400).json({
      ...
      code: "MISSING_ENV",
      message:
        "Preço não configurado para este produto/plano. Entre em contato com o suporte.",
      details: msg,
    });
  }
  ...
  try {
    ...
    try {
      ...
      console.error(
        "[asaas-create-payment] Erro ao criar cliente:",
        customerError,
      );
      return res.status(500).json({
        status: "error",
    ...
    if (billingType === "CREDIT_CARD" && creditCard) {
      ...
      const isValidCpfCnpjForCard =
        cpfCnpjForCard.length === 11 || cpfCnpjForCard.length === 14;
    ...
    const finalPayment = await asaasClient.createPayment(paymentData);
    ...
    await upsertZapfarmOrderFromPayment(finalPayment, {
    ...
    if (profile?.id) {
      ...
      if (supabaseUrl && supabaseKey) {
        ...
        if (req.body.cpfCnpj)
          newCache.cpfCnpj = String(req.body.cpfCnpj).replace(/\D/g, "");
        if (req.body.cep)
          newCache.cep = String(req.body.cep).replace(/\D/g, "");
        if (req.body.endereco) newCache.endereco = String(req.body.endereco);
        if (req.body.enderecoNumero)
          newCache.enderecoNumero = String(req.body.enderecoNumero);
        if (req.body.complemento)
          newCache.complemento = String(req.body.complemento);
        ...
        if (Object.keys(newCache).length > 0) {
          const { data: current } = await supabase
            .from("profiles")
            .select("checkout_cache")
            .eq("id", profile.id)
            .maybeSingle();
          const existing =
            (current?.checkout_cache as Record<string, string>) || {};
          await supabase
            .from("profiles")
            .update({
    ...
    const pixDetails =
      billingType === "PIX"
        ? await resolvePixTransaction(finalPayment, {
            loggerPrefix: "[asaas-create-payment]",
          })
        : null;
    const paymentLink =
      finalPayment.paymentLink || finalPayment.invoiceUrl || null;

    if (billingType === "PIX" && !pixDetails) {
      console.warn(
        "[asaas-create-payment] PIX criado sem QR imediato, devolvendo fallback:",
        {
      ...
      return res.status(200).json({
        ...
        warning:
          "QR Code PIX ainda nao ficou pronto no Asaas. Continue nesta pagina ou use o link de contingencia.",
        paymentLink,
        retryable: true,
      });
    }
