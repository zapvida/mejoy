...
import { requestSpecialistChannel, resolveMobileActor } from '@/lib/mobile/service';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const response = await requestSpecialistChannel({
    ...
    const message = error instanceof Error ? error.message : 'Erro ao solicitar canal premium';
