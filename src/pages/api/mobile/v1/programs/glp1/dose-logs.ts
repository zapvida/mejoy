...
import { createDoseLog, resolveMobileActor } from '@/lib/mobile/service';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const response = await createDoseLog({
    ...
    return res.status(400).json({
      error: error instanceof Error ? error.message : 'Erro ao registrar dose GLP-1',
    });
  }
}
