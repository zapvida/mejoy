...
import { z } from "zod";
import { serverEnv, isFeatureEnabled } from "@/lib/env";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<TtsResponse>) {
  ...
  // BEGIN patch tts-secure disabled → 200
  const enabled = process.env.TTS_ENABLED === "1";
  if (!enabled) {
    console.log(`[${reqId}] TTS disabled by feature flag`);
    return res.status(200).json({ 
      ok: true, 
      enabled: false,
      message: "Serviço de áudio temporariamente indisponível. Tente novamente mais tarde."
    });
  }
  // END patch
