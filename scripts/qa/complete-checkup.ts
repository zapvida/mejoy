#!/usr/bin/env node
// scripts/qa/complete-checkup.ts
...
class AlloeHealthChecker {
  ...
  private async testPremiumFeatures() {
  ...
  private async testAPIEndpoint(name: string, path: string, method: string): Promise<void> {
    const startTime = Date.now();
  ...
  private async testPDFGeneration(name: string, path: string): Promise<void> {
    const startTime = Date.now();
  ...
  private async testPerformanceEndpoint(name: string, path: string): Promise<void> {
    const startTime = Date.now();
  ...
  private async testSecurityEndpoint(name: string, path: string, method: string): Promise<void> {
    const startTime = Date.now();
  ...
  private makeRequest(path: string, method: string = 'GET'): Promise<{ statusCode: number; headers: any; body: string }> {
