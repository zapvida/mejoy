...
OUT_DIR = ROOT / "codex-artifacts" / "social-launch-v2"
...
def render_post(post: dict) -> Path:
    ...
    draw.text((bx + 29, by + 13), button_text, font=button_font, fill="white")

    note = "Acompanhamento médico individual"
    note_font = font(22, bold=False)
    nb = draw.textbbox((0, 0), note, font=note_font)
    nw = nb[2] - nb[0]
    draw.text((CANVAS_W - 72 - nw, by + 16), note, font=note_font, fill=MUTED_GREEN)
